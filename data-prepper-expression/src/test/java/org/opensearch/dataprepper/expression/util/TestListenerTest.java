...
class TestListenerTest {
    ...
    private void inStatement(final Runnable callback) {
    ...
    private void inExpression(final Runnable callback) {
        listener.enterEveryRule(parserRuleCtx);
    ...
    private void inOperatorExpression(final Runnable lhs, final TerminalNode operator, final Runnable rhs) {
        listener.enterEveryRule(parserRuleCtx);
        listener.enterExpression(expressionCtx);
        lhs.run();
    ...
    private void inPrimary(final Runnable callback) {
        listener.enterEveryRule(parserRuleCtx);
    ...
    private void inLiteral(final Runnable callback) {
        listener.enterEveryRule(parserRuleCtx);
    ...
    private void inListInitializer(final Runnable ... callbacks) {
    ...
    private Runnable withException(final Consumer<Runnable> in, final Runnable callback) {
        return () -> {
    ...
    private void testToken(final String token, final String result, final int type) {
        ...
        inStatement(() ->
                inExpression(() ->
                        inPrimary(() ->
                                inLiteral(() -> listener.visitTerminal(node))
                        )
                )
        );
    ...
    private void testOperator(final String operand, final int operandType, final String operator, final int operatorType) {
        ...
        inStatement(() ->
                inOperatorExpression(
                        () -> inPrimary(() ->
                                inLiteral(() -> listener.visitTerminal(operandNode))),
                        operatorNode,
                        () -> inPrimary(() ->
                                inLiteral(() -> listener.visitTerminal(operandNode)))
                )
    ...
    public void testInvalidExpressionRule() {
        ...
        inStatement(
                withException(this::inExpression, () -> listener.visitErrorNode(node))
        );
...
//        );
    ...
    public void testListRule() {
        ...
        final Runnable expression = () -> inExpression(() ->
                inPrimary(() ->
                        inLiteral(() -> listener.visitTerminal(integerNode))
                )
        );
